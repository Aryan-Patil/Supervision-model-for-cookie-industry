...
def shape_not_found():
    call(["python", "shape_not_found.py"])


def circle_val():
    call(["python", "circle-test.py"])


def triangle_val():
    call(["python", "triangle-test.py"])


def square_val():
    call(["python", "square-test.py"])
        ...
        for line in f:
            ...
            if line and line[0] != '#' and (',' in line or '=' in line):
                if ',' in line:
                    ...
                    continue
                if item in 'camera_id camera_width camera_height camera_frame_rate camera_fourcc' \
                           ' auto_percent auto_threshold auto_blur norm_alpha norm_beta'.split():
                    try:
                        exec(f'{item}={value}')
    ...
    if key_flags['config']:
        ...
        for c in contours:
            ...
            if (xlen < 56 and xlen > 53 and ylen > 53 and ylen < 56):
                draw.add_text(frame0, f'Circle Accept', x3 + 10, y2 - 40, center=True, color='green')
                draw.rect(frame0, x1, y1, x2, y2, weight=2, color='green')
                a1 = 1
            elif (xlen < 51 and xlen > 46 and ylen > 46 and ylen < 51):
                draw.add_text(frame0, f'Triangle Accept', x3 + 10, y2 - 40, center=True, color='green')
                draw.rect(frame0, x1, y1, x2, y2, weight=2, color='green')
                a1 = 2
            elif (xlen < 42 and xlen > 39 and ylen > 39 and ylen < 42):
                draw.add_text(frame0, f'Square Accept', x3 + 10, y2 - 40, center=True, color='green')
                draw.rect(frame0, x1, y1, x2, y2, weight=2, color='green')
                a1 = 3
            else:
